...
SUBROUTINE initialize_self_consistent
  implicit none
  integer :: i, j, k, omega, omega1, omega2

  !------- Initialization of the Schmidt basis -------------------
  do i = 1, nbasis
    do omega = -MxOmegaBasis, MxOmegaBasis
      if(omega/=0) then
        OriginalBasis(i, omega) = omega**(-1.d0*i)
      else
        OriginalBasis(i, omega) = 0.d0
      endif
    enddo
  enddo

  do omega1= -MxOmegaBasis, MxOmegaBasis
    do omega2= -MxOmegaBasis, MxOmegaBasis
      if(omega1/=0 .and. omega2/=0) then
        do i = 1, nbasis
          do j = 1, nbasis
            k = (i-1)*nbasis + j
            OriginalBasisGamma(k, omega1, omega2)=omega1**(-1.d0*i)*omega2**(-1.d0*j)
          enddo
        enddo
        OriginalBasisGamma(nbasisGamma-1, omega1, omega2)=omega1**(-1.d0)
        OriginalBasisGamma(nbasisGamma, omega1, omega2)=omega2**(-1.d0)

        ReweightBasis(omega1, omega2)=abs(omega1**(-1.d0) *omega2**(-1.d0))
      else
        OriginalBasisGamma(:, omega1, omega2) = 0.d0
        ReweightBasis(omega1, omega2) = 0.d0
      endif
    enddo
  enddo

  call calculate_basis(nbasis, MxOmegaG1+1, MxOmegaG2, GCoefP, GCoefN)
  call test_basis(nbasis, MxOmegaG1+1, MxOmegaG2, GCoefP, GCoefN)

  call calculate_basis(nbasis, MxOmegaW1+1, MxOmegaW2, WCoefP, WCoefN)
  call test_basis(nbasis, MxOmegaW1+1, MxOmegaW2, WCoefP, WCoefN)

  call calculate_basis(nbasis, MxOmegaGamG1+1, MxOmegaGamG2, GamGCoefP, GamGCoefN)
  call test_basis(nbasis, MxOmegaGamG1+1, MxOmegaGamG2, GamGCoefP, GamGCoefN)

  call calculate_Gamma_basis(nbasisGamma, MxOmegaGamG1+1, MxOmegaGamG2, GamCoefPN, &
    & GamCoefNP, GamCoefPPR, GamCoefNNR, GamCoefPPL, GamCoefNNL)
  call test_basis_Gamma(nbasisGamma, MxOmegaGamG1+1, MxOmegaGamG2, GamCoefPN, &
    & GamCoefNP, GamCoefPPR, GamCoefNNR, GamCoefPPL, GamCoefNNL)
...
SUBROUTINE initialize_G
  implicit none
  integer :: typ, omega, ib, nw
  double precision :: G0I
  ...
  do typ = 1, ntypG
    do omega = -MxOmegaG1, MxOmegaG1
      GI(typ, omega) = weight_G0up(omega, typ)
    enddo

    do ib = 1, nbasis
      do nw = MxOmegaG1+1, MxOmegaG2
        G0I = weight_G0up(nw, typ)
        GITailP(typ, ib) = GITailP(typ, ib) +weight_basis(GCoefP(ib,:),nw)*G0I
      enddo

      do nw = -MxOmegaG2, -MxOmegaG1-1
        G0I = weight_G0up(nw, typ)
        GITailN(typ, ib) = GITailN(typ, ib) +weight_basis(GCoefN(ib,:),nw)*G0I
      enddo
    enddo
  enddo
...
DOUBLE PRECISION FUNCTION weight_G0up(omega1, typ1)
  implicit none
  integer, intent(in)  :: omega1, typ1  
  double precision     :: muc, omegac 
  muc = Mu(1)*pi/(2.d0*Beta)
  omegac = (omega1*pi2+pi)/Beta
  weight_G0up = 1.d0/(muc+omegac)
END FUNCTION weight_G0up
