...
SUBROUTINE calculate_Polar
  ...
  do p = 0, Vol-1
    avePi(:) = (0.d0, 0.d0)
    do omega = 0, MxT-1
      avePi(omega)= dcmplx(0.5d0*(real(Polar(p, omega))+real(Polar(p, MxT-1-omega))), 0.5d0*( &
        & dimag(Polar(p, omega))+dimag(Polar(p, MxT-1-omega))))
    enddo
    Polar(p, :) = avePi(:)
  enddo
...
SUBROUTINE calculate_Sigma
  ...
  do omega = 0, MxT-1
    aveSigma(omega)= dcmplx(0.5d0*(real(Sigma(omega))+real(Sigma(MxT-1-omega))), 0.5d0*( &
      & dimag(Sigma(omega))-dimag(Sigma(MxT-1-omega))))
  enddo
