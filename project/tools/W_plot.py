...
for i in range(len(BoldSigma)):
    for key in target:
        ...
        ax.plot(tau, BoldSigma[i][key][0][0,0,:].real, label="{0}, Order{1}".format(key, (0,0)))
        ax.plot(tau, BoldSigma[i][key][0][1,0,:].real, label="{0}, Order{1}".format(key, (1,0)))
        ax.plot(tau, BoldSigma[i][key][0][1,1,:].real, label="{0}, Order{1}".format(key, (1,1)))
