...
SUBROUTINE markov(IsToss)
  implicit none
  integer :: iorder, istep,isamp,i,MaxSamp,iblck
  ...
  TimeRatio(1:3) = 1.d0
  do iorder = 4, MCOrder
    TimeRatio(iorder) = TimeRatio(iorder-1)*2.d0
  enddo
  ...
  do while(.true.)
    ...
    if(mod(iblck, NBLCKCHECK)==0) then
      ...
      if(mc_version/=file_version) then
        ...
        if(IFSLASH) then
          ...
          call slash_statistics(ENDOFDAYSLASH)

          call LogFile%WriteLine("after slash, the mc steps are:"+str(imc))
          IFSLASH = .false.
          call write_input(.false., file_version)
        endif
      endif
    ...
    if(mod(iblck,NBLCKREWEI)==0) then
...
SUBROUTINE recalculate_Reweighting
  implicit none
  integer :: i
  double precision :: x, xworm, xphy
  
  call LogFile%WriteStamp()
  call LogFile%WriteLine("Reweighting order of diagrams...")

  xworm = SUM(GamWormOrder(0:MCOrder))
...
SUBROUTINE measure
  ...
  if(IsWormPresent) then
    Z_worm=Z_worm+1.d0
    GamWormOrder(Order) = GamWormOrder(Order) + 1.d0/(CoefOfWorm*CoefOfWeight(Order))
  endif
  ...
  if(.not. IsWormPresent) then
    ...
    GamOrder(Order) = GamOrder(Order) + 1.d0/CoefOfWeight(Order)
    ...
    if(if_inside_Vol(D, dGamL(1:D), dr)) then

      dr = convert_r(D, L(1:D), GamL(1:D), dr)
      ...
      Norm(MCOrder+2) = Norm(MCOrder+2) + 1.d0
      Quan(MCOrder+2) = Quan(MCOrder+2) + real(Phase)/factorM
    endif
    ...
    if(sumt==NWLn .and. sumd==NVertex) then
      Quan(MCOrder+Order+3) = Quan(MCOrder+Order+3) + 1.d0/CoefOfWeight(Order)
      Norm(MCOrder+Order+3) = Norm(MCOrder+Order+3) + 1.d0
    endif
    !=============================================================
  endif
  ...
  if(ibin==1) then
    do ibasis = 1, NBasisGam
      wbasis = (1.d0/dble(MxT))**2.d0*weight_basis_Gam(CoefGam &
        & (0:BasisOrderGam,0:BasisOrderGam, ibasis,ibin), (dble(dt1)+0.5d0)/dble(MxT), &
        & (dble(dt2)+0.5d0)/dble(MxT))
...
SUBROUTINE slash_statistics(ratio)
  implicit none
  double precision :: ratio
  
  imc = imc/ratio
  GamNorm = GamNorm/ratio
  Z_normal = GamNorm/ratio

  GamMC(:, :, :) = GamMC(:, :, :)/ratio
  ReGamSqMC(:, :, :) = ReGamSqMC(:, :, :)/ratio
  ImGamSqMC(:, :, :) = ImGamSqMC(:, :, :)/ratio

  GamMCBasis(:, :, :, :, :) = GamMCBasis(:, :, :, :, :)/ratio
  ReGamSqBasis(:, :, :, :, :) = ReGamSqBasis(:, :, :, :, :)/ratio
  ImGamSqBasis(:, :, :, :, :) = ImGamSqBasis(:, :, :, :, :)/ratio
  return 
END SUBROUTINE slash_statistics
